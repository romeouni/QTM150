...
```
The x axis is not organised by any logical metric, it's alphabetical. The most common degree is High School.

## 5.

```{r}
sum(gss$sex %in% "FEMALE" & (gss$degree %in% "JUNIOR COLLEGE" | 
                             gss$degree %in% "BACHELOR" | 
                             gss$degree %in% "GRADUATE"))
```
...
mean(gss$age[gss$degree3==">HS"], na.rm=TRUE)
```
The average age for <HS is 52.94, the average age for HIGH SCHOOL is 43.42, the average age for >HS is 43.84.
